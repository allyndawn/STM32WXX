...
bool GPSTask::processBuffer() {
	...
	new_time.min = this->intFromString( scratchpad[1], 2, 2 );
	new_time.sec = this->intFromString( scratchpad[1], 4, 2 );

	// Scratchpad Entry #9: Date stamp (ddmmyy)
	if ( strlen( scratchpad[9] ) != 6 ) {
		return false;
	}
	new_time.year = this->intFromString( scratchpad[9], 4, 2 );
	...
	if ( 'S' == scratchpad[4][0] ) {
		new_location.latitude.degrees = - new_location.latitude.degrees;
	}
	...
	if ( 'E' == scratchpad[4][0] ) {
		new_location.longitude.degrees = - new_location.longitude.degrees;
	}
